...
public void setup() {
	...
	pg.colorMode(HSB, 360, 1.0f, 1.0f, 1.0f);
...
public void draw() {
	...
	time_pos = map(PApplet.parseFloat(song.position())/1000.0f, 0.0f, song_duration, 0, total_pixels);
...
public void subdivisionLines(JSONArray subdiv, float line_wgt, int line_col){
	...
	for (int i = 0; i < subdiv.size(); ++i) {
		...
		point_pos = map(pos_time, 0.0f, song_duration, 0, total_pixels);
		//println(point_pos);
...
public void drawSegments(JSONArray segs){
	...
	for (int i = 0; i < segs.size()-1; ++i) {
		...
		float point_pos1 = map(o1.getFloat("start"), 0.0f, song_duration, 0, total_pixels);
		float point_pos2 = map(o2.getFloat("start"), 0.0f, song_duration, 0, total_pixels);
		...
		pg.fill(0, 1.0f, 1.0f, confidence_alpha); //red
		pg.noStroke();
